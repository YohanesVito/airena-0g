 ...
 * 2. If below 0.4 0G, tops up from PRIVATE_KEY (deployer) to 0.5 0G.
 *    The 0.5 0G headroom covers the 0.001 0G registration fee plus
 *    follow-on bets and gas across multiple demo rehearsals without
 *    needing to re-fund between runs.
...
if (!RPC || !DEPLOYER_PK || !REGISTRY) {
  console.error(
    "[demo] Missing env: NEXT_PUBLIC_RPC_URL / PRIVATE_KEY / NEXT_PUBLIC_BOT_REGISTRY_ADDRESS"
  );
  process.exit(1);
}
